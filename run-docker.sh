...
if [ -n "$DEBUG" ]; then
    ...
    if [ $MODE -eq 1 ]; then
        valgrind --leak-check=full --show-leak-kinds=all valgrind --leak-check=full --show-leak-kinds=all ./JasmineGraph $PROFILE $MODE $MASTERIP $WORKERS $WORKERIP $ENABLE_NMON
    else
        valgrind --leak-check=full --show-leak-kinds=all valgrind --leak-check=full --show-leak-kinds=all ./JasmineGraph $PROFILE $MODE $HOST_NAME $MASTERIP $SERVER_PORT $SERVER_DATA_PORT $ENABLE_NMON
    fi
fi
