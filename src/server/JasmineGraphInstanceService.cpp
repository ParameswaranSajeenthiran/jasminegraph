 ...
 json response;
    ...
    FaissStore* faissStore = FaissStore::getInstance(std::stoi(Utils::getJasmineGraphProperty("org.jasminegraph.vectorstore.dimension")),nodeManager.getDbPrefix() + "_faiss.index");
        ...
        for (const auto currentPath : currentPaths) {
            ...
            vector<float> emb_;
            try{
                emb_ = faissStore->getEmbeddingById(lastNodeId);

            }
            catch (exception& e)
            {
                instance_logger.error("Error retrieving embedding for node ID: " + lastNodeId + ". Exception: " + e.what());
                continue;
            }

            if (emb_.empty()) {
                instance_logger.error("No embedding found for node ID: " + lastNodeId);
                continue;
            }
    ...
    // }
        ...
        instance_logger.error("Error occurred: " + std::string(e.what()));
    } 
